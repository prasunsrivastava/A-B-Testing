...
* Evaluation Metrics:
    ...
    2. Net Conversion - This metric measures how many users continue past the free trial period. Since, we expect the enrollment to be affected by the question about the time commitment, this metric should also be affected. The net conversion should not decrease even after this change.
...
* Fraction of Traffic - We will divert 100% of the traffic for the test as the change is not risky because we are providing additional information to the users in order to provide a better user experience. Also, the experiment does not put any personal information of the users at risk and we do not collect any additional personal information. On the part of the participants, it would not cost them any money. So, there is no reason to restrict the experiment for a subset of the users.
...
#### Invariant Metric
* Number of User IDs - Since, we require equal number of participants in the experiment and control group, number of User IDs will be used as an invariant metric.
